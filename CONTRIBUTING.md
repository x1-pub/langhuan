...
1. Fork 仓库并创建特性分支：`git checkout -b feat/awesome-feature`
2. 安装依赖：`pnpm install`
3. 按需启动 `pnpm dev:client` / `pnpm dev:server` 进行开发
4. 提交改动文件 (将依次执行 ESLint --fix 与 Prettier)
5. 提交 PR 前同步最新的 `master` 分支，解决冲突
...
- 统一使用 TypeScript
- 依赖注入或实例化逻辑请放在 `server/src`、`client/src` 各自的入口文件
