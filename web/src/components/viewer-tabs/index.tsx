...
const ViewerTabs: React.FC<{ children: ReactNode }> = ({ children }) => {
  const { wind, setWind, active, setActive } = useMain()

  const tabItems: TabsProps['items'] = wind.map(c => ({
  ...
  const handleRemove = (targetKey: React.MouseEvent | React.KeyboardEvent | string, action: string) => {
    if (action === 'add') {
      return
    }
