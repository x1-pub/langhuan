...
interface PartitionModalProps {
  visible: boolean;
  editingPartition: PartitionData | null;
  loading: boolean;
  onCancel: () => void;
  onSubmit: (values: PartitionData) => Promise<void>;
}
...
const PartitionModal: React.FC<PartitionModalProps> = ({
  ...
  // 处理分区类型变化
  const handlePartitionTypeChange = (type: PartitionType) => {
    // 根据分区类型清空相关字段
    if ([PartitionType.HASH, PartitionType.KEY, PartitionType.LINEAR_HASH, PartitionType.LINEAR_KEY].includes(type)) {
      form.setFieldsValue({ value: undefined });
    }
    if ([PartitionType.RANGE_COLUMNS, PartitionType.LIST_COLUMNS].includes(type)) {
