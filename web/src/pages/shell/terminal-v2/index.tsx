...
const TerminalV2: React.FC<TerminalProps> = (props) => {
  ...
  const calculateSuggestionPosition = (cursorPos: number) => {
    ...
    try {
      ...
      const adjustedLeft = left + suggestionWidth > containerWidth
  ...
  const applySuggestion = (suggestion: string) => {
    ...
    const newBeforeCursor = beforeCursor.substring(0, beforeCursor.length - currentWord.length) + suggestion;
    ...
    <div className={styles.terminalContainer} onClick={handleTerminalClick}>
      <div className={styles.terminalHeader}>
        ...
        <div className={styles.terminalControls}>
          <button className={`${styles.terminalBtn} ${styles.clear}`} onClick={() => setOutput([''])}>
            清空
          </button>
        </div>
      </div>
      ...
      <div className={styles.terminalContent}>
        {output.map((line, index) => (
          <div key={index} className={styles.terminalLine} dangerouslySetInnerHTML={{ __html:  ansiToHtml(line)}} />
        ))}
